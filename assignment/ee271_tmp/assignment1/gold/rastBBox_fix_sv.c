			  ...
			  )
{
  ...
  // for each vertex, find whether its x and y
  // is larger than the current max or less than
  // the current min
  int numVertices = poly.vertices;
  for(int i = 0; i < numVertices; i++)
  {
    if (poly.v[i].x[0] > max_x)
      max_x = poly.v[i].x[0];
    if (poly.v[i].x[0] < min_x)
      min_x = poly.v[i].x[0];

    if(poly.v[i].x[1] > max_y)
      max_y = poly.v[i].x[1];
    if(poly.v[i].x[1] < min_y)
      min_y = poly.v[i].x[1];
  }
  
  // round the values to the subsample grid
  ...
  // if larger or smaller than the width and height
  // of the screen, truncate to it
  ur_x = (ur_x > screen_w) ? screen_w : ur_x;
  ur_y = (ur_y > screen_h) ? screen_h : ur_y;
  ll_y = (ll_y < 0) ? 0 : ll_y;
  ll_x = (ll_x < 0) ? 0 : ll_x;

  // invalid if there is no width in the bbox (out of bounds)
			  ...
			  )   
{
  ...
  //
  // note that bool,true, and false are not in c
  // NOTE: I included stdbool.h to be allowed to use
  // the bool type but changing this is easy if we should not
  // do this
  int result = 0 ; // Default to miss state

  
  int numVertices = poly.q;

  // always have at least 3 vertices so normalize those to 0
  long x1 = poly.v[0].x[0] - s_x;
  long y1 = poly.v[0].x[1] - s_y;

  long x2 = poly.v[1].x[0] - s_x;
  long y2 = poly.v[1].x[1] - s_y;  
  
  long x3 = poly.v[2].x[0] - s_x;
  long y3 = poly.v[2].x[1] - s_y;
  ...
  // only set this if there is a 4th vertex
  if (numVertices == 4)
  {
    x4 = poly.v[3].x[0] - s_x;
    y4 = poly.v[3].x[1] - s_y;
  ...
  // v1 to v2
  bool r1 = (x1*y2-x2*y1 <= 0);
  // v2 to v3
  bool r2 = (x2*y3 - x3*y2 < 0);
  // v3 to v1

  // if 3 vertices end here
  if (numVertices == 3)
  {
    r3 = (x3*y1 - x1*y3 <= 0);
  }
  ...
  // if all to the right, this is true
  if(numVertices == 3)
    result = r1 & r2 & r3;
  else
    result = r1 & r2 & r3 & r4;
